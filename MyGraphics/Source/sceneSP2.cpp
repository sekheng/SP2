...
void sceneSP2::Init()
{
    ...
    Mtx44 projection;
    projection.SetToPerspective(75.f, static_cast<float>(screenWidth / screenHeight), 0.1f, 10000.f);
...
void sceneSP2::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
    ...
    {
        Mtx44 characterSpacing;
        characterSpacing.SetToTranslation(i + (forComicSans.eachCharSpace[i] / 23), 0, 0); //1.0f is the spacing of each character, you may change this value
        Mtx44 MVP = projectionStack.Top() * viewStack.Top() * modelStack.Top() * characterSpacing;
        glUniformMatrix4fv(m_parameters[U_MVP], 1, GL_FALSE, &MVP.a[0]);
